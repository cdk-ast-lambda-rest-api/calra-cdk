...
# def get_directory_decorators_by_endpoint(directory):
#     python_files = []
#     dictionary = {}
...
#         parsed_tree = parse_file(file)
#         file_id = file[len(directory)+len(os.sep):]
#         new_dict = get_decoratorsandfuncs_by_file(parsed_tree, id = file_id)
#         if has_decorators(parsed_tree):
#             for key, value in new_dict.items():
#                 #If an endpoint doesn't exist, create it and assign it a lambda id with it's decorator
#                 #Else add new lambda to not overwrite previous lambdas
...
# def get_decoratorsandfuncs_by_file(parsed_tree, id = None):
#     return_dict = {} 
...
#                     # An HTTP request will always have an endpoint
#                     if decorator_name in {'GET', 'PUT', 'DELETE', 'POST'}:
#                         http_decorator = True #PENDIENTE USAR
#                         #First argument should be path
#                         endpoint_name = decorator.args[0].s 
                        
#                     args = [arg.s for arg in decorator.args]
#                     method_decorators.setdefault(decorator_name, []).extend(args) #Lo mete en una lista
#                 else:
#                     # Handle decorators without arguments
#                     decorator_name = ast.unparse(decorator).strip()
#                     # Aggregate metadata from all decorators
#                     method_decorators.setdefault(decorator_name, [])
#             method_decorators.update({'handler':func_name})
#             lambda_dict[lambda_id] = method_decorators

#             #If an endpoint doesn't exist, create it and assign it a lambda id with it's decorator
#             #Else add new lambda to not overwrite previous lambdas
#             if endpoint_name not in return_dict.keys():
#                 return_dict[endpoint_name] = lambda_dict
#             else:
#                 return_dict[endpoint_name][lambda_id] = method_decorators
