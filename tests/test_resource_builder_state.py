...
def test_custom_registry_add_get_and_unknown_key(builder, kind):
    ...
    if kind == "vpc":
        ...
        if kind == "environment" and value == "":
            with pytest.raises(KeyError):  # current falsey lookup behavior
                builder.get_custom_environment("shared")
        else:
...
def test_custom_environment_accepts_zero_but_falsey_lookup_fails(builder):
    builder.add_custom_environment("zero", 0)
    assert builder.custom_environments["zero"] == 0
    with pytest.raises(KeyError):
        builder.get_custom_environment("zero")
